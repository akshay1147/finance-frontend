...
export default function Home() {
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 flex flex-col justify-between relative overflow-hidden">
      ...
      {/* Hero Section */}
      <main className="max-w-4xl mx-auto px-6 py-12 md:py-20 flex-grow flex flex-col items-center justify-center text-center space-y-8 z-10">
        <div className="space-y-4">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight bg-gradient-to-r from-white via-slate-100 to-slate-400 bg-clip-text text-transparent leading-tight max-w-2xl mx-auto">
            Operational Finance & Payment Gateways
          </h1>
          <p className="text-xs md:text-sm text-slate-400 max-w-lg mx-auto leading-relaxed">
            Experience the unified financial workspace for Portal 6. Manage invoices, reconcile bank clearing ledgers, and execute PCI-DSS compliant credit card checkout pipelines.
          </p>
        </div>

        {/* Dual Actions */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-2xl pt-4">
          {/* Internal Portal */}
          <Link
            href="/dashboard"
          ...
          >
            <div>
              ...
              <h3 className="text-sm font-bold text-white mb-2">Internal Finance Portal</h3>
      ...
      <footer className="px-6 py-4 border-t border-slate-900 text-center text-[10px] text-slate-500">
        © 2026 LTI Technologies Inc. Portal 6 Core Operations Interface. Built by Jayashree V (P6-A4).
      </footer>
    </div>
  );
}
